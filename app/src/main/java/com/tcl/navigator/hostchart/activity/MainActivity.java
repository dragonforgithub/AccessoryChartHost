...
public class MainActivity extends AppCompatActivity implements UsbDetachedReceiver.UsbDetachedListener, OpenDevicesReceiver.OpenDevicesListener, View.OnClickListener {
    ...
    private byte[]  mBytes            = new byte[1024*16]; //使用bulkTransfer传输API时，最大 BUFFER SIZE 就是 16384（16k）
    ...
    private void initAccessory(UsbDevice usbDevice) {
        ...
        int supportVersion = usbDeviceConnection.controlTransfer(UsbConstants.USB_DIR_IN | UsbConstants.USB_TYPE_VENDOR, 51, 0, 0, bytes, 16, 100);
        MyApplication.printLogDebug("Support version : " + supportVersion);

        //2.如果设备返回所支持的协议版本，则向设备发送含标识字符串信息的控制请求。该信息让设备可以确定适合配件的应用（如果没有适合配件的应用，则向用户呈现一个网址）。该控制请求为端点 0 上的请求（适用每个字符串 ID）
        ...
        //3.发送控制请求，要求设备以配件模式启动。该控制请求为端点 0 上的请求.
        usbDeviceConnection.controlTransfer(UsbConstants.USB_DIR_OUT | UsbConstants.USB_TYPE_VENDOR, 53, 0, 0, new byte[]{}, 0, 100);
    ...
    private void initStringControlTransfer(UsbDeviceConnection deviceConnection, int index, String string) {
        deviceConnection.controlTransfer(UsbConstants.USB_DIR_OUT | UsbConstants.USB_TYPE_VENDOR, 52, 0, index, string.getBytes(), string.length(), 100);
    }
                ...
                while (mToggle) {
                    ...
                    if (!values.isEmpty()) {
                        for (UsbDevice usbDevice : values) {
                            ...
                            int productId = usbDevice.getProductId();
                            if (vendorId == 0x18D1 &&
                                    (productId == 0x2D00 || productId == 0x2D01 || productId == 0x2D04)) {
                                if (mUsbManager.hasPermission(usbDevice)) {
                ...
                while (isReceiverMessage) {
                    ...
                    if (mUsbDeviceConnection != null && mUsbEndpointIn != null) {
                        ...
                        if(i >= 1024){
                            rcvBytes += i;
                            if(totalTime > 1000){
                                MyApplication.printLogDebug("Transfer speed : " + rcvBytes/1000/1000 + "MB/s");
                                rcvBytes = 0;
